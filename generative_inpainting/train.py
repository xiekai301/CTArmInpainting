...
# from inpaint_model_c3jpg import InpaintCAModel
from inpaint_model import InpaintCAModel


def multigpu_graph_def(model, FLAGS, data, gpu_id=0, loss_type='g'):  # 已完整阅读
...
if __name__ == "__main__":
    ...
    # FLAGS = ng.Config('inpaint_celebahq.yml')
    img_shapes = FLAGS.img_shapes
    with open(FLAGS.data_flist[FLAGS.dataset][0]) as f:
        fnames = f.read().splitlines()
    if FLAGS.guided:  # False
        # fnames = [(fname, fname[:-4] + '_edge.jpg') for fname in fnames]
        fnames = [(fname, fname[:-4] + '_edge.png') for fname in fnames]
        # fnames = [fname.split(', ') for fname in fnames]
        img_shapes = [img_shapes, img_shapes]
    data = ng.data.DataFromFNames(
        fnames, img_shapes, random_crop=FLAGS.random_crop,
        nthreads=FLAGS.num_cpus_per_job)  # data准备, 注意这里已经裁剪乘256, 单通道进入data_from_fnames.py尽心修改

    print('debug2')

    images = data.data_pipeline(FLAGS.batch_size)  # data按batch_size输出
    # main model
    model = InpaintCAModel()
    g_vars, d_vars, losses = model.build_graph_with_losses(FLAGS, images)  # 这里summary是False,这里的losses似乎没用到
    # validation images
    if FLAGS.val:
        ...
        if FLAGS.guided:
            ...
            val_fnames = [
                (fname, fname[:-4] + '_edge.png') for fname in val_fnames]
        # progress monitor by visualizing static images
        for i in range(FLAGS.static_view_size):
            static_fnames = val_fnames[i:i+1]  # '.../1038.png'
            static_images = ng.data.DataFromFNames(
                static_fnames, img_shapes, nthreads=1,
                random_crop=FLAGS.random_crop).data_pipeline(1)  # (1,384,384,3)
            static_inpainted_images = model.build_static_infer_graph(  # TODO: 何时调用的, 不应该一开始就调用啊
                FLAGS, static_images, name='static_view/%d' % i)  # tensorboard输出的static_view, 注意有page 3
    # training settings
    lr = tf.get_variable(
        'lr', shape=[], trainable=False,
        initializer=tf.constant_initializer(1e-4))  # 1e-4
    d_optimizer = tf.train.AdamOptimizer(lr, beta1=0.5, beta2=0.999)  # beta1=0.5
