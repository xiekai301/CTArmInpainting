...
if __name__ == '__main__':
    ...
    guidance = np.zeros((h, w), np.uint8)
    ...
    cv2.createTrackbar('radius', 'image_masked', 5, 50, lambda x: None)
    cv2.createTrackbar('radius_edge', 'image_masked', 0, 5, lambda x: None)
    cv2.setMouseCallback('image_masked', draw_circle)
    # cv2.setMouseCallback('image_masked', draw_edge)

    while True:
        image_masked = np.clip(image[0, :, :, 0] * (1-mask[0, :, :, 0])
                               + guidance[0, :, :, 0], -1, 1) / 2 + 1 / 2  # * mask[0, :, :, 0]
        cv2.imshow('image_masked', image_masked)
        radius = cv2.getTrackbarPos('radius', 'image_masked')
        radius_edge = cv2.getTrackbarPos('radius_edge', 'image_masked')
        ...
        if k == 27:  # Esc
            ...
            guidance = np.zeros((1, h, w, 1), np.uint8)
        elif k == 13:  # Enter

            input_image = np.concatenate([image, guidance * 255, mask * 255], axis=2)
            result = sess.run(output, feed_dict={input_image_ph: input_image})

            cv2.imshow('result', result[0, :, :, 0] / 2 + 1 / 2)
