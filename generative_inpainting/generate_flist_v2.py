...
parser.add_argument('--folder_path', default='./training_data/CTArmNpy', type=str,
...
if __name__ == "__main__":
    ...
    validation_dirs = os.listdir(args.folder_path + "/test_forTPS")
    ...
    for training_dir in sorted(training_dirs):
        ...
        training_folder = sorted(os.listdir(args.folder_path + "/training" + "/" + training_dir))
        for training_item in training_folder:
            # modify to full path -> directory
            if 'npy' in training_item:
                training_item = args.folder_path + "/training" + "/" + training_dir + "/" + training_item
                training_file_names.append(training_item)

    # append all files into 2 lists
    for validation_dir in sorted(validation_dirs):
        # append each file into the list file names
        validation_folder = os.listdir(args.folder_path + "/test_forTPS" + "/" + validation_dir)
        for validation_item in sorted(validation_folder):
            # modify to full path -> directory
            if 'npy' in validation_item:
                validation_item = args.folder_path + "/test_forTPS" + "/" + validation_dir + "/" + validation_item
