...
from inpaint_ops_c3jpg import gen_conv, gen_deconv, dis_conv
from inpaint_ops_c3jpg import random_bbox, bbox2mask, local_patch, brush_stroke_mask
from inpaint_ops_c3jpg import resize_mask_like, contextual_attention
...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, reuse=False,
        ...
        with tf.variable_scope(name, reuse=reuse), \
            ...
            x = gen_conv(x, 4 * cnum, 3, rate=3, name='conv9_atrous')  # (6, 96, 96, 96)
            x = gen_conv(x, 4 * cnum, 3, rate=4, name='conv10_atrous')  # (6, 96, 96, 96)
            ...
            x = gen_conv(x, 3, 3, 1, activation=None, name='conv17')  # (6, 384, 384, 3)  # change 3 to 1
            ...
            x = gen_conv(x, 4 * cnum, 3, rate=3, name='xconv9_atrous')  # (6, 96, 96, 96)
            x = gen_conv(x, 4 * cnum, 3, rate=4, name='xconv10_atrous')  # (6, 96, 96, 96)
            x_hallu = x  # (6, 96, 96, 96)  # hallucinate
            # attention branch
            ...
            pm = x  # (6, 96, 96, 96)  # parallel module
            ...
            x = gen_conv(x, 3, 3, 1, activation=None, name='allconv17')  # (6, 384, 384, 3), 这里改维度 change 3 to 1
            x = tf.nn.tanh(x)  # (6, 384, 384, 3)
    ...
    def build_graph_with_losses(
        ...
        if summary:
            ...
            images_summary(  # TODO 查看打印的输出图像
                tf.concat(viz_img, axis=2),
                'raw_incomplete_predicted_complete', FLAGS.viz_max_out, image_width=FLAGS.img_shapes[0],
                mean=FLAGS.mean_value, std=FLAGS.std_value, low=FLAGS.low,
                high=FLAGS.high)  # 对应上tensorboard中的输出, 但这个有两个(g和d调用两次)
    ...
    def build_infer_graph(self, FLAGS, batch_data, bbox=None, name='val'):
        ...
        if FLAGS.guided:
            ...
            # viz_img = [batch_pos, batch_incomplete, batch_complete]
            viz_img = [batch_pos, batch_incomplete, x1, x2, batch_complete]  # 'list', 三个(1,384,384,3)
            # viz_img = [tf.tile(batch_pos, [1,1,1,3]), tf.tile(batch_incomplete, [1,1,1,3]), tf.tile(batch_complete, [1,1,1,3])]  # 'list', 三个(1,384,384,3) change
        if offset_flow is not None:
            viz_img.append(
                resize(offset_flow, scale=4,
                       func=tf.image.resize_bilinear))
        ...
        images_summary(
            ...
            image_width=FLAGS.img_shapes[0], mean=FLAGS.mean_value, std=FLAGS.std_value, low=FLAGS.low, high=FLAGS.high)
