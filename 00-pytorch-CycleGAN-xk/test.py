...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        patient_name = img_path[0].split('/')[-2]
        out_patient_path = os.path.join(out_file, patient_name)
        ...
        result = fakeB*data['mask'].cpu().numpy() + data['B'].cpu().numpy()*(1-data['mask'].cpu().numpy())
        # np.save(os.path.join(out_patient_path, img_name[:-4] + '.npy'), np.uint16(np.around((result[0,0,:,:] + 1) / 2 * 2500)))
        np.save(os.path.join(out_patient_path, img_name[:-4] + '.npy'), np.uint16(np.around((result[0,0,:,:] + 1) / 2 * 2500)))

        # if i % 1 == 0:  # save images to an HTML file
        #     print('processing (%04d)-th image... %s' % (i, img_path))
    t_comp = (time.time() - iter_start_time) / len(dataset)
    print(t_comp)
